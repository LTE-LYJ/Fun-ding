...
public class ProjectReportDao {
	...
	public List<ProjectReport> getProjectReportList(Connection con, String field, String query, int page) {
		...
		String sql = "SELECT * FROM( " + "				 SELECT ROWNUM NUM, N.* "
				+ "			FROM (SELECT A.*, B.MEM_NAME " + "                FROM PROJECT_REPORT A"
				+ "                INNER JOIN MEMBER B ON MEM_NO = PRJ_REPORT_WRITER " + "            WHERE " + field
				+ " LIKE ? ORDER BY CREATE_DATE DESC) N  " + "				WHERE STATUS = 'Y'" + "				 ) "
		...
		try {
			...
			while (rs.next()) {
				...
				ProjectReport prjReport = new ProjectReport(prjReportNo, prjReportTitle, prjReportContent,
						prjReportWriter, writerName, count, createDate, status, files, prj_no, answer);
	...
	public ProjectReport getProjectReport(Connection con, int projectReportNo) {
		...
		String sql = "SELECT * FROM PROJECT_REPORT A" + "	INNER JOIN MEMBER B ON MEM_NO = PRJ_REPORT_WRITER"
				+ " WHERE PRJ_REPORT_NO=?";
		PreparedStatement st = null;
		ResultSet rs = null;
		try {
			...
			if (rs.next()) {
				...
				prjReport = new ProjectReport(prjReportNo, prjReportTitle, prjReportContent, prjReportWriter,
						writerName, count, createDate, status, files, prj_no, answer);
	...
	public ProjectReport getNextProjectReport(Connection con, int id) {
		...
		String sql = "SELECT PRJ_REPORT_NO, PRJ_REPORT_TITLE FROM PROJECT_REPORT "
				+ "				WHERE PRJ_REPORT_NO = ( " + "				SELECT PRJ_REPORT_NO FROM PROJECT_REPORT "
		...
		try {
			...
			if (rs.next()) {
				...
				prjReport = new ProjectReport(prjReportNo, prjReportTitle);
	...
	public int insertProjectReport(Connection con, ProjectReport p) {
		...
		String sql = "INSERT INTO PROJECT_REPORT "
				+ "VALUES(PRJ_REPORT_NO_SEQ.NEXTVAL, ? , ? , ?, DEFAULT,DEFAULT,  DEFAULT, ?, ?, DEFAULT)";
		PreparedStatement st = null;
	...
	public ProjectReport selectProjectReport(Connection con, int ProjectReportNo) {
		ProjectReport prjReport = null;
		String sql = "SELECT MEM_NAME, P.*  " + " FROM PROJECT_REPORT P "
				+ "INNER JOIN MEMBER ON MEM_NO = PRJ_REPORT_WRITER " + "WHERE PRJ_REPORT_NO=? ";
	...
	public int answerProjectReport(Connection con, PrAnswer p) {
		int result = 0;
		String sql = "INSERT INTO PROJECT_REPORT_ANSWER " + " VALUES(?, ? , ? , ?, DEFAULT,DEFAULT,?, ?)";
		PreparedStatement st = null;

		try {
			st = con.prepareStatement(sql);
			st.setInt(1, p.getPrjReportNo());
			st.setString(2, p.getPrjReportTitle());
			st.setString(3, p.getPrjReportContent());
			st.setInt(4, Integer.parseInt(p.getWriterName()));
			st.setInt(5, p.getPrjNo());
			st.setString(6, p.getFiles());
	...
	public int answerCheck(Connection con, int prjReportNo) {
		int result = 0;
		PreparedStatement st = null;

		String sql = "UPDATE PROJECT_REPORT SET ANSWER = 'Y' WHERE PRJ_REPORT_NO=?";

		try {
			st = con.prepareStatement(sql);
			st.setInt(1, prjReportNo);
	...
	public int updateAnswerCount(Connection con, int prjReportNo) {
		int result = 0;
		PreparedStatement st = null;

		String sql = "UPDATE PROJECT_REPORT_ANSWER SET COUNT = COUNT+1 WHERE PRJ_REPORT_NO=?";
	...
	public PrAnswer getPrAnswer(Connection con, int p) {
		PrAnswer prAnswer = null;

		String sql = "SELECT * FROM PROJECT_REPORT_ANSWER A" + "	INNER JOIN MEMBER B ON MEM_NO = PRJ_REPORT_WRITER"
				+ " WHERE PRJ_REPORT_NO=?";
		PreparedStatement st = null;
		ResultSet rs = null;
		try {
			...
			if (rs.next()) {
				...
				prAnswer = new PrAnswer(prjReportNo, prjReportTitle, prjReportContent, prjReportWriter,
						writerName, count, createDate, files, prj_no);
	...
	public int deleteAnswer(Connection con, int prjReportNo) {
		int result = 0;
		String sql = "DELETE PROJECT_REPORT_ANSWER WHERE PRJ_REPORT_NO=?";
		PreparedStatement st = null;
	...
	public int prjDeleteAnswer(Connection con, int prjReportNo) {
		int result = 0;
		PreparedStatement st = null;

		String sql = "UPDATE PROJECT_REPORT SET ANSWER = 'N' WHERE PRJ_REPORT_NO=?";
