...
<body>
    ...
    <div class="outer">
        <h1 style="margin-left:10px">마감/실패 프로젝트 둘러보기</h1>
		...
		<div class="pagingArea" align="center">
			<!-- 맨 처음으로 (<<) -->
			<button onclick="location.href='<%= request.getContextPath() %>/viewClose.pr?currentPage=1'"> &lt;&lt; </button>
		
			<!-- 이전페이지로(<) -->
			<%if(currentPage == 1){ %>
			<button disabled> &lt; </button>
			<%}else{ %>
			<button onclick="location.href='<%= request.getContextPath() %>/viewClose.pr?currentPage=<%= currentPage-1 %>'"> &lt; </button>
			<%} %>
			...
			<%for(int p=startPage; p<=endPage; p++){ %>
				...
				<%}else{ %>
				<button onclick="location.href='<%= request.getContextPath() %>/viewClose.pr?currentPage=<%= p %>'"> <%= p %> </button>
				<%} %>
			...
			<!-- 맨 끝으로 (>>) -->
			<button onclick="location.href='<%= request.getContextPath() %>/viewClose.pr?currentPage=<%=maxPage%>'"> &gt;&gt; </button>
		</div> 
	</div>
